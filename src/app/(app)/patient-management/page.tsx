...
import { Search, PlusCircle, User, ShieldAlert, FilePlus } from 'lucide-react';
...
function PatientManagementPageComponent() {
  ...
  useEffect(() => {
    if (token) {
        // Debounce search, or fetch initial list if no search term
        const debounce = setTimeout(() => {
            fetchPatients(searchTerm);
        }, searchTerm ? 300 : 0);
        return () => clearTimeout(debounce);
    }
  }, [token, searchTerm, fetchPatients]);
